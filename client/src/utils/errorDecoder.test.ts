...
describe("extractErrorCode", () => {
    ...
    it("parses deeply nested JSON payload with code", () => {
        ...
        expect(extractErrorCode(payload)).toBe(8);
    });

    it("parses deeply nested JSON payload with string representation", () => {
        const payload = JSON.stringify({
            data: {
                message: "WasmVm error: Error(Contract, #9)"
            }
        });
        expect(extractErrorCode(payload)).toBe(9);
    });
});
