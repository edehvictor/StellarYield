...
/** Full claim set covering all sort dimensions. */
...
describe("claimBatchOrdering", () => {
  describe("normalizeClaimBatch", () => {
    ...
    it("returns an empty batch for empty input", () => {
      expect(normalizeClaimBatch([])).toEqual([]);
    });

    it("rejects a duplicate claim with a typed error", () => {
      const dup = makeClaim();
      const batch = [makeClaim(), dup];
      try {
        ...
        expect(typed.claim).toEqual(dup);
    ...
    it("treats claims as distinct when any sort dimension differs", () => {
      const batch = [
        makeClaim({ campaignId: "2026-W20" }),
        makeClaim({ account: "GZZZZ" }),
        makeClaim({ asset: "USDC" }),
        makeClaim({ claimIndex: 1 }),
      ];
      const ordered = normalizeClaimBatch(batch);
      expect(ordered).toHaveLength(4);
    });

    it("mixed batch: dedupes, orders, and reports the exact duplicate key", () => {
      const mixed = shuffle([
  ...
  describe("compareClaims / claimOrderKey", () => {
    it("compares by claimIndex numerically", () => {
      const low = makeClaim({ claimIndex: 2 });
      const high = makeClaim({ claimIndex: 10 });
      expect(compareClaims(low, high)).toBeLessThan(0);
      expect(compareClaims(high, low)).toBeGreaterThan(0);
    });
