...
// ── Backtest calibration checks ─────────────────────────────────────────────

describe("predictApy backtest calibration checks", () => {
  const fixtureData = [
    5.0, 5.2, 5.5, 5.4, 5.8, 6.0, 6.2, 6.1, 6.5, 6.8, 7.0, 6.9, 7.1, 7.5,
    7.3, 7.8, 8.0, 7.9, 8.2, 8.5, 8.3, 8.1, 7.8, 7.5, 7.2, 7.0, 6.8, 6.5,
  ].map((apy, i) => ({
  ...
  it("measures forecast quality against held-out windows", () => {
    // Hold out the last 7 days
    const trainingData = fixtureData.slice(0, 21);
    const holdOutData = fixtureData.slice(21);

    const result = predictApy("Blend", trainingData, 7);
    expect(result.predictions).toHaveLength(7);

    // Calculate mean absolute error (MAE)
    let totalError = 0;
    for (let i = 0; i < 7; i++) {
      const actual = holdOutData[i].apy;
      const predicted = result.predictions[i].predictedApy;
      totalError += Math.abs(actual - predicted);
    }
    const mae = totalError / 7;
    // Ensure the prediction is reasonably close
    expect(mae).toBeLessThan(1.5); 
  });

  it("actual forecast error falls within confidence bands", () => {
    const trainingData = fixtureData.slice(0, 21);
    const holdOutData = fixtureData.slice(21);

    const result = predictApy("Blend", trainingData, 7);

    let withinBandsCount = 0;
    for (let i = 0; i < 7; i++) {
      const actual = holdOutData[i].apy;
      const { lowerApy, upperApy } = result.predictions[i];
      if (actual >= lowerApy && actual <= upperApy) {
        withinBandsCount++;
      }
    }
    // Most actual points should fall within the predicted confidence bands
    expect(withinBandsCount).toBeGreaterThanOrEqual(4); 
  });

  it("confidence calibration detects unstable forecasts (drift)", () => {
    const volatileFixture = fixtureData.map((d, i) => ({
      ...d,
      apy: d.apy + (i % 2 === 0 ? 5 : -5) // Inject high volatility
    }));

    const trainingData = volatileFixture.slice(0, 21);
    const result = predictApy("Blend", trainingData, 7);
    
    // High volatility should decrease confidence
    expect(result.confidenceInputs.volatilityPct).toBeGreaterThan(0.2); 
    expect(result.predictions[0].confidence).toBeLessThan(0.9);
  });
});
