...
describe("GET /api/health/readiness", () => {
  ...
  it("returns 503 with DB unavailable", async () => {
    jest.resetModules();
    jest.doMock("@prisma/client", () => ({
      PrismaClient: jest.fn().mockImplementation(() => ({
    ...
    expect(res.status).toBe(503);
    expect(res.body.dependencies.database.status).toBe("unavailable");
    expect(res.body.dependencies.database.errorCode).toBe("DB_UNREACHABLE");
    expect(res.body.dependencies.database.retryable).toBe(true);
  });

  it("returns degraded Horizon when response is slow", async () => {
    mockHorizonCall.mockImplementation(
      () => new Promise((resolve) => setTimeout(() => resolve({ records: [{ sequence: 105 }] }), 150)),
    );
    mockRpcGetNetwork.mockResolvedValue({ passphrase: "Test SDF Network ; September 2025" });

    const res = await request(app).get("/api/health/readiness");

    expect(res.body.dependencies.horizon.status).toBe("degraded");
    expect(res.body.dependencies.horizon.retryable).toBe(true);
  });
});
