...
router.get('/attribution/:walletAddress', async (req, res) => {
  try {
    ...
    if (!validation.valid) {
      return res.status(400).json(
        errorEnvelope('VALIDATION_ERROR', validation.error || 'Invalid request parameters', 'analytics/attribution'),
      );
    }
...
router.post('/attribution/config', async (req, res) => {
  try {
    ...
    res.json(
      successEnvelope(
        { message: 'Attribution configuration updated', config: portfolioAttributionEngine.getConfig() },
...
router.post('/compatibility/config', async (req, res) => {
  try {
    ...
    res.json(
      successEnvelope(
        { message: 'Compatibility configuration updated', config: protocolCompatibilityEngine.getConfig() },
...
router.get('/health/alerts', async (req, res) => {
  try {
    ...
    res.json(
      successEnvelope(
        { alerts, criticalCount: alerts.length, totalStrategies: healthScores.length },
...
router.post('/health/config', async (req, res) => {
  try {
    ...
    res.json(
      successEnvelope(
        { message: 'Health configuration updated', config: strategyHealthEngine.getConfig() },
...
router.post('/health/batch', async (req, res) => {
  try {
    ...
    if (!Array.isArray(strategyIds) || strategyIds.length === 0) {
      return res.status(400).json(
        errorEnvelope('VALIDATION_ERROR', 'strategyIds must be a non-empty array', 'analytics/health/batch'),
      );
    }

    const healthScores = await strategyHealthEngine.getHealthScores(strategyIds);
    res.json(successEnvelope(healthScores.map(formatHealthScore), 'analytics/health/batch'));
  } catch (error) {
    res.status(500).json(
      errorEnvelope(
        'INTERNAL_ERROR',
...
router.get('/health/:strategyId', async (req, res) => {
  try {
    ...
    res.json(successEnvelope(formatHealthScore(healthScore), 'analytics/health'));
  } catch (error) {
...
router.post('/reliability/batch', async (req, res) => {
  try {
    ...
    if (!Array.isArray(providers) || providers.length === 0) {
      return res.status(400).json(
        errorEnvelope('VALIDATION_ERROR', 'providers must be a non-empty array', 'analytics/reliability/batch'),
      );
    }
...
router.post('/reliability/config', async (req, res) => {
  try {
    ...
    res.json(
      successEnvelope(
        { message: 'Reliability configuration updated', config: yieldReliabilityEngine.getConfig() },
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res.json(successEnvelope(report, 'analytics/recommendation-stability/compare'));
    } catch (error) {
      res.status(500).json(
        errorEnvelope(
          'INTERNAL_ERROR',
