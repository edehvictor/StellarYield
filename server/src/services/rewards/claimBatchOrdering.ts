/**
 * Server-side deterministic ordering for batched Merkle claim submissions.
 *
 * Mirrors the contract in `backend/rewards/src/claimBatchOrdering.ts` so
 * the server can normalize claim batches (proofs + previews) without a
 * cross-package dependency. The two implementations must stay in sync:
 *
