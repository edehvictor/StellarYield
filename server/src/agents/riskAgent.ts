...
export async function assessProtocolRisk(input: ProtocolInput): Promise<RiskReport> {
  try {
    ...
    if (
      ...
      !parsed.factors ||
      typeof parsed.factors.smartContractRisk !== "number" ||
      typeof parsed.factors.governanceRisk !== "number" ||
      typeof parsed.factors.marketRisk !== "number" ||
      typeof parsed.factors.sentimentScore !== "number"
    ) {
