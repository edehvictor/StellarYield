...
> [!IMPORTANT]
> **Database Migrations:** Our CI pipeline strictly verifies that all schema changes have been committed as migrations. Before pushing, always run `npx prisma migrate dev` in the `server` directory and commit the generated migration files in `prisma/migrations/`. Do not use `npx prisma db push` as it bypasses migration history and will fail the CI check.
